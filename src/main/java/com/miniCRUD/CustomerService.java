...
//			return;
	...
	public void data() {
		...
		try {
			...
			while (rs.next()) {
				...
				CustomerStatus status = CustomerStatus.valueOf(a);

				c.setStatus(status);
				c.setEmail(rs.getString(5));

				c.setBirthDate(rs.getDate(6));
				save(c);
			}
			connection.close();
	...
	public void setdata(Customer customer) {
		...
		try {
			...
			String sqlDate=df.format(customer.getBirthDate());



			statement.executeUpdate(
					"INSERT INTO customers (firstName,lastName, status,email,birthDate) " +
							"VALUES ('"+customer.getFirstName()+"','"+customer.getLastName()+"','"
							+customer.getStatus()+"','"+customer.getEmail()+"',"+" DATE '"+sqlDate+"')");
	...
	public void updatedata(Customer customer) {
		...
		try {
			...
			statement.executeUpdate("UPDATE customers SET firstName ='"+ customer.getFirstName()+"',lastName ='"+
					customer.getLastName()+"',status='"+ customer.getStatus()+"',email='"+customer.getEmail()+"',birthDate='"+customer.getBirthDate()+
	...
	public void deletedata(Customer customer) {
		...
		try {
			...
			connection = DriverManager.getConnection(url, "root", "");
			statement = connection.createStatement();
			statement.executeUpdate("DELETE FROM customers  WHERE id="+customer.getId()+"");
